...
void loop() {
  lcd.clear();
  int Voltage0=analogRead(A0)/2.05;
  int Voltage1=analogRead(A1)/2.05;
  // print the top bar graph
  lcd.setCursor(0,0);
  // draw lower spaces as solids
  for (int pos=0; pos < Voltage0/10; pos++) {  
  lcd.write(5);
  }
  // print the lower bar graph
  lcd.setCursor(0,1);
  // draw lower spaces as solids
  for (int pos=0; pos < Voltage1/10; pos++) {  
  lcd.write(5);
  }
  lcd.setCursor(Voltage0/10, 0);
  if (Voltage0%10/2 == 0) lcd.write(1);
  if (Voltage0%10/2 == 1) lcd.write(2);
  if (Voltage0%10/2 == 2) lcd.write(3);
  if (Voltage0%10/2 == 3) lcd.write(4);
  if (Voltage0%10/2 == 4) lcd.write(5);
  lcd.setCursor(Voltage1/10, 1);
  if (Voltage1%10/2 == 0) lcd.write(1);
  if (Voltage1%10/2 == 1) lcd.write(2);
  if (Voltage1%10/2 == 2) lcd.write(3);
  if (Voltage1%10/2 == 3) lcd.write(4);
  if (Voltage1%10/2 == 4) lcd.write(5);
  // print the voltages:
  lcd.setCursor(0, 0);
  lcd.print(Voltage0);
  lcd.setCursor(0, 1);
