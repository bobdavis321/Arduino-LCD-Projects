...
void loop() {
  ...
  lcd.setCursor(8, 0);
  lcd.print(Voltage);
  // overwrite first digit and add a period
  lcd.setCursor(7, 0);
  ...
  for (int pos=0; pos < Voltage/10; pos++) {  
  lcd.write(5);
  }
  lcd.setCursor(Voltage/10, 1);
  if (Voltage%10/2 == 0) lcd.write(1);
  if (Voltage%10/2 == 1) lcd.write(2);
  if (Voltage%10/2 == 2) lcd.write(3);
  if (Voltage%10/2 == 3) lcd.write(4);
  if (Voltage%10/2 == 4) lcd.write(5);
  delay(500);
}
